...
const returnTelegramPost = async (req) => {
            ...
            title: z
              .object({
                $regex: z
                  .string()
                  .describe(
                    "The regular expression pattern to match the title"
                  ),
                $options: z
                  .string()
                  .describe(
  ...
  const result = await executeQueryAndGeneratePost(
    `find a movie with title '${req.body.message.text}'`
  );
  console.log("Final Result:", JSON.stringify(result, null, 2));
  return result; // Directly return the result instead of using res.json()
};

export { returnTelegramPost };
